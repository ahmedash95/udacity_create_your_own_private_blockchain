...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(time > currentTime - 300_000) {
                if(bitcoinMessage.verify(message, address, signature)) {
                    let block = new BlockClass.Block({owner: address, data: star});
                    await self._addBlock(block);
                    resolve(block);
                } else {
                    reject(Error("Block message is not valid"));
                }
            } else {
                reject(Error("Block is not valid due to timeout"));
            }
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for(let i = 1; i < self.height; i++) {
                ...
                if (! await block.validate()) {
                    ...
                    errorLog.push(`Block height [${block.height}] previousBlockHash is not valid`);
                }
            }
